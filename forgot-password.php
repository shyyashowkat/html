...
if (isset($_POST['send_otp'])) {
     if(trim($_POST["register"]) == 'student'){
      ...
      $query = "SELECT email FROM students WHERE email = '$to'";
...
$mail->Body    = 'Your One Time Password (OTP) for reset password at learnintern.com is: '. $otp .' <br><br>--- for
...
if(!$mail->send()) {
  ...
  $_SESSION['type'] = "students";
          ...
          echo "<script>alert('No registered Student email Found! Provide valid Email-ID.');</script>";
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
        if(trim($_POST["register"]) == 'trainer'){
          $to = mysqli_real_escape_string($conn, $_POST['email']);
         $query = "SELECT email FROM trainers WHERE email = '$to'";
         $result = mysqli_query($conn, $query);
           if($result){
             $otp = rand(100000,999999);
             require 'php-mailer/PHPMailerAutoload.php';
   ...
   if(!$mail->send()) {
     ...
     $_SESSION['type'] = "trainers";
             ...
             echo "<script>alert('No registered Trainer email Found! Provide valid Email-ID.');</script>";
             echo "<script>location.href='/forgot-password.php';</script>";
           }
           }
}
...
if (isset($_POST['reset_password'])) {
  ...
  if ($otp == $_SESSION['otp']) {
   ...
   $query2 = "UPDATE trainers SET password = '$pass' WHERE email = '$email'";}
   if ($_SESSION['type'] == "students") {
  $query2 = "UPDATE students SET password = '$pass' WHERE email = '$email'";}
        ...
        <div class="section-content">
          ...
          <section>
            ...
            <?php }
              ...
              ?>
              <div class="section-content">
                <div class="row enter-email">
                  <div class="col-xs-12 col-sm-8 col-md-8">
                    <form name="editprofile-form" method="post" action="forgot-password.php" onsubmit="return validate()">
                      <div class="icon-box mb-0 p-0">
                        <a href="#" class="icon icon-bordered icon-rounded icon-sm pull-left mb-0 mr-10">
                          <i class="fa fa-key"></i>
                        </a>
                      ...
                      <div class="row">
                        <div class="form-check col-md-6">
                          <input type="radio" name="register" id="1" class="form-check-input" required value="student">
                          <label for="1" class="form-check-label"> I am a student</label> &nbsp;&nbsp;
&nbsp; &nbsp;                         <input type="radio" name="register" id="2" class="form-check-input" value="trainer" required>
                          <label for="2" class="form-check-label"> I am a trainer</label>
                        </div>
                        </div>
                      <div class="row">
                        <div class="form-group col-md-6">
                        ...
                        <div class="row">
                          <div class="col-md-6 err-msg text-danger">
                          Email is required !
                          </div>
                          <script>
                            onload = document.querySelector('.err-msg').style.display = "none";
                      ...
                      <div class="form-group">
                        <button name="send_otp" class="btn btn-dark btn-lg mt-15" type="submit">Send</button>
                      </div>
                    </form>
                  </div>
